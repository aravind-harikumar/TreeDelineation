...
addParameter(arg, 'method', 'fixedRadius', @(x) any(validatestring(x, {'fixedRadius', 'allometricRadius', 'hMaxima','LevelSet'})));
...
switch arg.Results.method
    ...
    case 'LevelSet'    
        SizeFilter = 11; Sigma = 1;
        img_max_filtered = medfilt2(chm, [SizeFilter SizeFilter]);
        % figure, surf(img_max_filtered)
        gaussian_filter = fspecial('gaussian', SizeFilter, Sigma);
        img_max_filtered=conv2(img_max_filtered,gaussian_filter,'same');

        MinDistance = 1; height_threshold = 5;
        idx_lm = LevelSet_Seed_Extraction(chm,img_max_filtered,MinDistance,height_threshold);
        val_lm = chm(find(idx_lm>0));
        [row_lm, col_lm] = ind2sub(size(idx_lm), find(idx_lm>0));        
end
...
%xyh = xyh(xyh(:,3)>arg.Results.minHeightDifference,:); % For Testing Only
...
%%%%%%%%%%%%%%%%%%% --added additionally for handling partial tree crowns
%%%%%%%%%%%%%%%%%%% in the boundaries of the plot
Ln = size(chm,1); Br = size(chm,2);
Center = [round(Ln/2,0) round(Br/2,0)]; 
theeta = 1:5:360;

RadiusINCHM = arg.Results.plotRadius/arg.Results.cellResolution;

X_OUTER = round((RadiusINCHM*0.9)*cosd(theeta)+Center(1));
Y_OUTER = round((RadiusINCHM*0.9)*sind(theeta)+Center(2));

X_OUTER1 = round((RadiusINCHM*0.8)*cosd(theeta)+Center(1));
Y_OUTER1 = round((RadiusINCHM*0.8)*sind(theeta)+Center(2));

IndxCol = [ones(size(crh,1),1); zeros(size(X_OUTER,2),1)];
crh = [crh; [X_OUTER' Y_OUTER' repmat(200,size(X_OUTER,2),1)] ];
crh = [crh IndxCol];

sizeCHMBeforeOutPointAdd = size(crh,1);
%%%%%%%%%%%%%%%%%%%
...
if arg.Results.fig
    ...
    f1=figure;
    set(f1, 'Position', [45 240 500 400]);
    imagesc(chm);
    colormap('gray');
    colorbar;
    hold on;
    %plot(crh(crh(:,4)==1,1), crh(crh(:,4)==1,2), 'rx');
    axis equal tight;
    ...
    %plot(X_OUTER1,Y_OUTER1,'-.','LineWidth',2);
    axis off; 
    %export_fig(gcf, 'aa.jpg');
end
