...
if ~arg.Results.headerOnly && r.header(phb_skeys.n_point_records).value > 0
    ...
    % <<<<< Code line added to fix incorrect point count issue>>>>
    r.header(phb_skeys.n_point_records).value = size(blob,1)/double(r.header(phb_skeys.point_data_record_length).value);
